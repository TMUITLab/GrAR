...
def get_edge_embeddings(combined_embed,graph):

	n_nodes = int(combined_embed.shape[0] / 2) #right now assume graphs are same size
	...
	for e in graph.edges():
		if(e[0]<n_nodes):
			edge_embed1.append(np.append(combined_embed[e[0]] , combined_embed[e[1]]))
			edge_embed1.append(np.append(combined_embed[e[1]], combined_embed[e[0]]))
			map1.append([e[0],e[1]])
			map1.append([e[1], e[0]])
		else:
			edge_embed2.append(np.append(combined_embed[e[0]] , combined_embed[e[1]]))
			edge_embed2.append(np.append(combined_embed[e[1]], combined_embed[e[0]]))
			map2.append([e[0], e[1]])
			map2.append([e[1], e[0]])
	edge_embed1 = np.array(edge_embed1);
	edge_embed2 = np.array(edge_embed2);
	return edge_embed1, edge_embed2,map1,map2,
...
def score_embeddings_matrices(embed1, embed2, topk = None, similarity_threshold = None, normalize = False, true_alignments = None, sim="cosine"):
	...
	#get_estimated_alignment_matrix(similarity_matrix, similarity_threshold, normalize)
